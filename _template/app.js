...
if (CONFIG.logos.partner.src) {
  ...
  document.getElementById("hero-partner-logo-wrap").outerHTML = `
    <div class="border-2 border-dashed border-neutral-3 rounded-2xl px-6 py-4 md:px-8 md:py-5 inline-flex items-center">
      <span class="text-subtitle-1 font-semibold uppercase tracking-wider text-neutral-4">Your Logo Here</span>
    </div>
  `;
}
...
document.getElementById("upgrades-label").textContent = CONFIG.upgrades.label;
document.getElementById("upgrades-intro").textContent = CONFIG.upgrades.intro;

const state = {
  scope: CONFIG.scope.default,
  provinces: new Set(),
  selectedUpgrades: new Set(), // upgrade ids the user has added
};

/* -- Foundation card (always included, not toggleable) -- */
const foundationCardEl = document.getElementById("foundation-card");
function renderFoundationCard() {
  const f = CONFIG.foundation;
  foundationCardEl.innerHTML = `
    <div class="rounded-2xl border-2 border-mb-navy bg-mb-navy p-6">
      <div class="flex items-start justify-between gap-4 mb-4">
        <h4 class="font-bold text-white text-lg">${f.name}</h4>
        <span class="text-xl font-bold tabular-nums text-white shrink-0">${fmt(f.price)}</span>
      </div>
      <ul class="space-y-3">
        ${f.includes.map((item) => `
          <li class="flex items-start gap-2.5">
            ...
            <span class="text-compact text-white/80"><span class="font-semibold text-white">${item.name}:</span> ${item.desc}</span>
          </li>
        `).join("")}
      </ul>
    </div>
...
/* -- Upgrade cards (toggleable) -- */
const upgradeCardsEl = document.getElementById("upgrade-cards");
function renderUpgradeCards() {
  upgradeCardsEl.innerHTML = "";
  CONFIG.upgrades.options.forEach((opt) => {
    const selected = state.selectedUpgrades.has(opt.id);
    const card = el("div", `option-card p-5 ${selected ? "selected" : ""}`, `
      <div class="flex items-start justify-between gap-3 mb-2">
        <span class="font-semibold text-mb-navy text-compact">${opt.name}</span>
        <span class="text-lg font-bold tabular-nums shrink-0 ${selected ? "text-mb-blue" : "text-mb-navy"}">+${fmt(opt.price)}</span>
      </div>
      <p class="text-subtitle-1 text-text-secondary leading-relaxed mb-2">${opt.desc}</p>
      ${opt.note ? `<p class="text-subtitle-1 text-text-muted italic">${opt.note}</p>` : ""}
      <div class="flex items-center gap-2 mt-3 text-subtitle-1 font-semibold ${selected ? "text-mb-blue" : "text-text-muted"}">
        <div class="checkbox-box" style="${selected ? "background:var(--mb-blue);border-color:var(--mb-blue)" : ""}">
          ${selected ? `<svg class="w-3.5 h-3.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg>` : ""}
        </div>
        <span>${selected ? "Added to your package" : "Add this upgrade"}</span>
      </div>
    `);
    card.addEventListener("click", () => {
      if (state.selectedUpgrades.has(opt.id)) state.selectedUpgrades.delete(opt.id);
      else state.selectedUpgrades.add(opt.id);
      renderAll();
    });
...
function currentPrice() {
  let total = CONFIG.foundation.price;
  CONFIG.upgrades.options.forEach((opt) => {
    if (state.selectedUpgrades.has(opt.id)) total += opt.price;
  });
  return total;
}
...
function renderSummary() {
  ...
  summaryEl.appendChild(el("div", "flex items-start justify-between gap-4", `
    <div class="flex items-center gap-2">
      <svg class="w-4 h-4 text-mb-blue shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg>
      <span class="text-compact text-white/90">${CONFIG.foundation.name}</span>
    </div>
    <span class="text-compact font-medium text-white/70 tabular-nums shrink-0">${fmt(CONFIG.foundation.price)}</span>
  `));

  CONFIG.upgrades.options.forEach((opt) => {
    if (!state.selectedUpgrades.has(opt.id)) return;
    summaryEl.appendChild(el("div", "flex items-start justify-between gap-4", `
      <div class="flex items-center gap-2">
        <svg class="w-4 h-4 text-mb-blue shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg>
        <span class="text-compact text-white/90">${opt.name}</span>
      </div>
      <span class="text-compact font-medium text-white/70 tabular-nums shrink-0">+${fmt(opt.price)}</span>
    `));
  });

  if (state.selectedUpgrades.size === 0) {
    summaryEl.appendChild(el("p", "text-subtitle-1 text-white/40 pt-1", "No upgrades added yet."));
  }
